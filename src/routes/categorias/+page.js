...
export async function load({ fetch }) {
	const [moviesResponse, categoriesResponse, movieCategoriesResponse] = await Promise.all([
		...
		return {
			...
			averageRating:
				categoryMovies.length > 0
					? categoryMovies.reduce((total, movie) => total + Number(movie.rating || 0), 0) / categoryMovies.length
					: 0,
